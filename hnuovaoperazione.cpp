    ...
    ui(new Ui::HnuovaOperazione)
{
    ...
    ui->deDataOperazione->setDateTime(QDateTime::currentDateTime());
...
bool HnuovaOperazione::saveNewLot(QString nl)
{
    ...
    data=ui->deDataOperazione->dateTime().toString("yyyy-MM-dd hh:mm:ss");
    ...
    QString sql="INSERT INTO lotdef(lot,prodotto,data,giacenza,um,scadenza,bolla,anagrafica,lot_fornitore,tipo,attivo,note) values (:lot,:prodotto,:data,:giacenza,:um,:scadenza,:bolla,:anagrafica,:lot_fornitore,:tipo,:attivo,:note)";
    q.prepare(sql);
    ...
    q.bindValue(":bolla",QVariant(bolla));
    ...
    q.bindValue(":note",QVariant(ui->leNote->text()));
...
bool HnuovaOperazione::saveOperationCarico()
{
  //ID=-1,lot=createID,data=now(),utente=user,IDprodotto=listaprodotti,azione=1,quantita,um,note
    ...
    QSqlQuery q(db);
    QString query ="SELECT createid_sf(:id,:data)";
    q.prepare(query);
    q.bindValue(":id",idprodotto);
    q.bindValue(":data",QVariant(ui->deDataOperazione->date().toString("yyyy-MM-dd")));
    q.exec();
    q.first();
...
//salvo l'operazione
    ...
    QString data=ui->deDataOperazione->dateTime().toString("yyyy-MM-dd hh:mm:ss");
...
QString HnuovaOperazione::getNewLot(QString prodotto)
{
    ...
    nl.exec("SELECT createid_sf("+ prodotto +", '"+ ui->deDataOperazione->date().toString("yyyy-MM-dd")+"')");
    nl.next();

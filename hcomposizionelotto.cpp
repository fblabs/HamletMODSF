...
HComposizioneLotto::HComposizioneLotto( HUser * p_user,QSqlDatabase pdb, int idLotto, QString descrizione, HComposizioneLotto *parentf,QWidget *parent) :
    QWidget(parent),
...
void HComposizioneLotto::removeComponent()
{
    int operation=ui->tableView->model()->index(ui->tableView->currentIndex().row(),1).data(0).toInt();
    db.transaction();
    QSqlQuery q(db);
    QString sql="delete from composizione_lot where operazione=:operazione";
    q.prepare(sql);
    q.bindValue(":operazione",operation);
    q.exec();
    mod->query().exec();
    if (QMessageBox::question(this,QApplication::applicationName(),"Confermere l'eliminazione?",QMessageBox::Ok|QMessageBox::Cancel)==QMessageBox::Ok)
    {
        db.commit();
    }
    else
...
void HComposizioneLotto::refresh()
{
    getLotComposition();
}
...
void HComposizioneLotto::on_pbAdd_clicked()
{
     ...
     HAddComponent *f=new HAddComponent(id,user,db);
     connect(f,SIGNAL(done()),this,SLOT(refresh()));
     f->show();
}

void HComposizioneLotto::on_pbPrint_clicked()
{
     print();
}

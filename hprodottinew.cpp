...
HProdottiNew::HProdottiNew(QSqlDatabase pdb,  HUser *puser, QWidget *parent) :
    QWidget(parent),
...
void HProdottiNew::on_tvProdotti_doubleClicked(const QModelIndex &index)
{
    ...
    int id = ui->tvProdotti->model()->index(index.row(),0).data(0).toInt();
    if (id>0)
    {
    HModProduct *f=new HModProduct(id,db);
    connect(f,SIGNAL(done()),this,SLOT(reloadProduct()));
    f->show();
    }else
    {

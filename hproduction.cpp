...
void HProduction::getRecipesForClient()
{
 ...
 // QString qs="select ricette.ID,prodotti.ID,prodotti.descrizione from prodotti, ricette, associazioni where ricette.ID=associazioni.ID_ricetta and prodotti.ID=ricette.ID_prodotto and associazioni.visualizza=1 and associazioni.visualizza=1 and associazioni.ID_cliente=:idcliente";
  ...
  else if(ui->rbImpasti->isChecked())
  {
      tipo=3;
  }
  else if(ui->rbRipieni->isChecked())
  {
      tipo=4;
  }

  qs="select ricette.ID,prodotti.ID,prodotti.descrizione from prodotti, ricette where  prodotti.ID=ricette.ID_prodotto and prodotti.tipo=:tipo order by prodotti.descrizione ASC";
...
{
    QSqlQuery qLotQr(db);
    QString sLot="SELECT createid_sf(:sprod,:data)";
    QString ret;
    qLotQr.prepare(sLot);
    qLotQr.bindValue(":sprod",QVariant(prod));
    qLotQr.bindValue(":data",ui->deDataProd->date().toString("yyyy-MM-dd"));
    qLotQr.exec();
    qLotQr.first();
...
// qDebug()<<scadenza.toString("yyyy-MM-dd");
    ...
    QString data=ui->deDataProd->dateTime().toString("yyyy-MM-dd hh:mm:ss");
...
int HProduction::lastInsertId()
{
 ...
 bool HProduction::saveOperation(int row,int action)//carico=1 - scarico= 2
 {
     ...
     data=ui->deDataProd->date().toString("yyyy-MM-dd");
     note="";
...
//creo un nuovo lotto
 ...
 qDebug()<<"saveNewLot"<<fb<<lotto;

  newlotid=lastInsertId();
  qDebug()<<"newlotid:" <<QString::number(newlotid);
 if(!fb)
   {
...
void HProduction::on_rbProdotti_toggled(bool checked)
{
    if (checked) getRecipesForClient();
}

void HProduction::on_rbImpasti_toggled(bool checked)
{
    if (checked) getRecipesForClient();
}

void HProduction::on_rbRipieni_toggled(bool checked)
{
    if (checked) getRecipesForClient();
}

void HProduction::on_cbClienti_currentIndexChanged(int index)
{

...
void HAddLotInProduction::lastLots()
{
    ...
    QString sql="select lotdef.ID,lotdef.lot,lotdef.prodotto,lotdef.giacenza,prodotti.allergenico from lotdef,prodotti where prodotti.ID=lotdef.prodotto and lotdef.prodotto=:prd and lotdef.attivo>0 ORDER by lotdef.data DESC LIMIT :quanti";
    qlots.prepare(sql);
...
{
    if (ui->leGiacenza->text().toDouble()<ui->dsbQt->value())
    {
        if(QMessageBox::warning(this,QApplication::applicationName(),"la giacenza è inferiore alla quantità da aggiungere! Continuare?",QMessageBox::Ok|QMessageBox::Cancel)==QMessageBox::Ok)
        {
            addLot();
            close();
        }
...
void HAddLotInProduction::on_lvLastLots_clicked(const QModelIndex &index)
{
    QModelIndex qtix=ui->lvLastLots->model()->index(index.row(),3);

   ui->leGiacenza->setText(QString::number(qtix.data().toDouble(),'f',3));
}
